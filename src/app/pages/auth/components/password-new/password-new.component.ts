...
export class PasswordNewComponent implements OnInit {
  ...
  validateForm(){
    ...
    if (!regex.test(this.form.controls['password'].value)){
      this.messageService.add({severity:'error', summary:'Algo anda mal', detail: 'La contraseña es muy débil.'});
      return false
    }
  ...
  onSubmitClick(){
      if (this.validateForm()){
        this.messageService.add({severity:'info', summary:'Operación en curso', detail: 'Hemos recibido tu petición'});
        let data = {
          token: this.token,
